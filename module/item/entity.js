 ...
 */
export default class ItemBFRPG extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.bfrpg.areaTargetTypes);
  }
  ...
  prepareData() {
    ...
    // Feat Items
      ...
      if ( act && (act.type === C.abilityActivationTypes.legendary) ) labels.featType = game.i18n.localize("bfrpg.LegendaryActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.lair) ) labels.featType = game.i18n.localize("bfrpg.LairActionLabel");
      else if ( act && act.type ) labels.featType = game.i18n.localize(data.damage.length ? "bfrpg.Attack" : "bfrpg.Action");
      else labels.featType = game.i18n.localize("bfrpg.Passive");
    }

    // Equipment Items
    else if ( itemData.type === "equipment" ) {
      labels.armor = data.armor.value ? `${data.armor.value} ${game.i18n.localize("bfrpg.AC")}` : "";
    }
...
//      labels.range = [rng.value, rng.long ? `/ ${rng.long}` : null, C.distanceUnits[rng.units]].filterJoin(" ");
      labels.range = [rng.short ? `${rng.short}` : null, rng.medium ? `/ ${rng.medium}` : null, , rng.long ? `/ ${rng.long}` : null, C.distanceUnits[rng.units]].filterJoin(" ");
      ...
      labels.recharge = `${game.i18n.localize("bfrpg.Recharge")} [${chg.value}${parseInt(chg.value) < 6 ? "+" : ""}]`;
    }
    ...
    if ( data.hasOwnProperty("actionType") ) {
      ...
      if ( save?.ability && !this.isOwned ) {
        ...
        labels.save = game.i18n.format("bfrpg.SaveDC", {dc: save.dc || "", ability: C.abilities[save.ability]});
      }
  ...
  async roll({configureDialog=true, rollMode=null, createMessage=true}={}) {
    ...
    const template = `systems/bfrpg/templates/chat/${templateType}-card.html`;
    ...
    if ( (this.data.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["bfrpg.itemData"] = this.data;
    }
  ...
  async _handleResourceConsumption({isCard=false, isAttack=false}={}) {
    ...
    const typeLabel = CONFIG.bfrpg.abilityConsumptionTypes[consume.type];
    ...
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("bfrpg.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( [null, undefined].includes(consumed) ) {
      ui.notifications.warn(game.i18n.format("bfrpg.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    let remaining = quantity - amount;
    if ( remaining < 0) {
      ui.notifications.warn(game.i18n.format("bfrpg.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async _rollFeat(configureDialog) {
    ...
    if ( consume && charge.value ) {
      if ( !charge.charged ) {
        ui.notifications.warn(game.i18n.format("bfrpg.ItemNoUses", {name: this.name}));
        return false;
      }
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      props.push(
        game.i18n.localize(data.equipped ? "bfrpg.Equipped" : "bfrpg.Unequipped"),
        game.i18n.localize(data.proficient ? "bfrpg.Proficient" : "bfrpg.NotProficient"),
      );
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.bfrpg.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.stealth.value ? game.i18n.localize("bfrpg.StealthDisadvantage") : null
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.bfrpg.weaponTypes[data.weaponType],
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.bfrpg.consumableTypes[data.consumableType],
      data.uses.value + "/" + data.uses.max + " " + game.i18n.localize("bfrpg.Charges")
    );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.bfrpg.abilities[data.ability] || null,
      CONFIG.bfrpg.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("bfrpg.ItemTypeLoot"),
      data.weight ? data.weight + " " + game.i18n.localize("bfrpg.AbbreviationLbs") : null
    );
  }
  ...
  async rollAttack(options={}) {
    const itemData = this.data.data;
    const actorData = this.actor.data.data;
    const flags = this.actor.data.flags.bfrpg || {};
    if ( !this.hasAttack ) {
      throw new Error("You may not place an Attack Roll with this Item.");
    }
    let title = `${this.name} - ${game.i18n.localize("bfrpg.AttackRoll")}`;
    const rollData = this.getRollData();

    // Define Roll bonuses
    const parts = [`@mod`];
    /*  // No proficiency in BFRPG
    ...
    */
    ...
    // Attack Bonus - BFRPG Specific
    switch ( itemData.actionType ) {
      case "mwak":
        parts.push(actorData.attributes.AttackBonusMelee.value);
        break;
      case "rwak":
        parts.push(actorData.attributes.AttackBonusRanged.value);
        break;
      case "msak":
        parts.push(actorData.attributes.AttackBonusMelee.value);
        break;
    ...
    const rollConfig = mergeObject({
      ...
      messageData: {"flags.bfrpg.roll": {type: "attack", itemId: this.id }}
    }, options);
  ...
  rollDamage({event, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const itemData = this.data.data;
    const actorData = this.actor.data.data;
    const messageData = {"flags.bfrpg.roll": {type: "damage", itemId: this.id }};
    ...
    const title = `${this.name} - ${game.i18n.localize("bfrpg.DamageRoll")}`;
    const rollConfig = {
      event: event,
      parts: parts,
      actor: this.actor,
    ...
    if ( versatile && itemData.damage.versatile ) {
      ...
      messageData["flags.bfrpg.roll"].versatile = true;
    }
    ...
    if ( itemData.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("bfrpg", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("bfrpg.OtherFormula")}`;
    ...
    roll.toMessage({
      ...
      messageData: {"flags.bfrpg.roll": {type: "other", itemId: this.id }}
    });
    return roll;
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("bfrpg.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "bfrpg.ItemRechargeSuccess" : "bfrpg.ItemRechargeFailure")}`,
  ...
  rollToolCheck(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("bfrpg.ToolCheck")}`;

    // Compose the roll data
    const rollConfig = mergeObject({
      parts: parts,
      data: rollData,
      template: "systems/bfrpg/templates/chat/tool-roll-dialog.html",
      title: title,
      speaker: ChatMessage.getSpeaker({actor: this.actor}),
      flavor: title,
      fastForward: true,
      ...
      halflingLucky: this.actor.getFlag("bfrpg", "halflingLucky" ) || false,
      messageData: {"flags.bfrpg.roll": {type: "tool", itemId: this.id }}
    }, options);
    rollConfig.event = options.event;
  ...
  static async _onChatCardAction(event) {
    ...
    const storedData = message.getFlag("bfrpg", "itemData");
    const item = storedData ? this.createOwned(storedData, actor) : actor.getOwnedItem(card.dataset.itemId);
    if ( !item ) {
      return ui.notifications.error(game.i18n.format("bfrpg.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name}))
    }
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("bfrpg.ActionWarningNoToken"));
    return targets;
  }
   ...
   * @param {ItemBFRPG} spell      The spell to be made into a scroll
   * @return {ItemBFRPG}           The created scroll consumable item
   * @private
   */
  static async createScrollFromSpell(spell) {

    // Get spell data
    const itemData = spell instanceof ItemBFRPG ? spell.data : spell;
    const {actionType, description, source, activation, duration, target, range, damage, save, level} = itemData.data;

    // Get scroll data
    const scrollUuid = CONFIG.bfrpg.spellScrollIds[level];
    ...
    const spellScrollData = mergeObject(scrollData, {
      name: `${game.i18n.localize("bfrpg.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      data: {
