/**
 * A standardized helper function for managing core BFRPG "d20 rolls"
 *
...
export async function d20Roll({parts=[], data={}, event={}, rollMode=null, template=null, title=null, speaker=null,
  ...
  const _roll = (parts, adv, form) => {
    ...
    if (adv === 1) {
      ...
      messageData.flavor += ` (${game.i18n.localize("bfrpg.Advantage")})`;
      if ( "flags.bfrpg.roll" in messageData ) messageData["flags.bfrpg.roll"].advantage = true;
      mods += "kh";
    }
    ...
    // Handle disadvantage
      ...
      nd = 2;
      messageData.flavor += ` (${game.i18n.localize("bfrpg.Disadvantage")})`;
      if ( "flags.bfrpg.roll" in messageData ) messageData["flags.bfrpg.roll"].disadvantage = true;
      mods += "kl";
    }
    ...
    if (ability && ability.value) {
      ...
      if (abl) {
        data.mod = abl.mod;
        messageData.flavor += ` (${CONFIG.bfrpg.abilities[data.ability]})`;
      }
    }
    ...
    if (reliableTalent && roll.dice[0].total < 10) {
      messageData.flavor += ` (${game.i18n.localize("bfrpg.FlagsReliableTalent")})`;
    }
    return roll;
  };
...
async function _d20RollDialog({template, title, parts, data, rollMode, dialogOptions, roll}={}) {
  ...
  // Render modal dialog
  template = template || "systems/bfrpg/templates/chat/roll-dialog.html";
  let dialogData = {
    formula: parts.join(" + "),
    data: data,
    rollMode: rollMode,
  ...
  return new Promise(resolve => {
    new Dialog({
      ...
      buttons: {
        advantage: {
          label: game.i18n.localize("bfrpg.Advantage"),
          callback: html => resolve(roll(parts, 1, html[0].querySelector("form")))
        },
        normal: {
          label: game.i18n.localize("bfrpg.Normal"),
          callback: html => resolve(roll(parts, 0, html[0].querySelector("form")))
        },
        disadvantage: {
          label: game.i18n.localize("bfrpg.Disadvantage"),
          callback: html => resolve(roll(parts, -1, html[0].querySelector("form")))
        }
      },
      default: "normal",
...
export async function damageRoll({parts, actor, data, event={}, rollMode=null, template, title, speaker, flavor,
  ...
  const _roll = function(parts, crit, form) {
    ...
    if ( crit === true ) {
      ...
      messageData.flavor += ` (${game.i18n.localize("bfrpg.Critical")})`;
      if ( "flags.bfrpg.roll" in messageData ) messageData["flags.bfrpg.roll"].critical = true;
    }
    ...
    new Dialog({
      ...
      buttons: {
        critical: {
          ...
          label: game.i18n.localize("bfrpg.CriticalHit"),
          callback: html => resolve(roll(parts, true, html[0].querySelector("form")))
        },
        normal: {
          label: game.i18n.localize(allowCritical ? "bfrpg.Normal" : "bfrpg.Roll"),
